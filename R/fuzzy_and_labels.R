...
convert_labels_table_to_spec_table <- function(.input_tbl,
  ...
  db_child_info <- get_target_type_info_below(.url = db_loc,
                                              .target_types = c("MEASURE", "COUNT", "FIELD")) |> dplyr::rename_with( ~
                                                                                                                       paste0("child_", .x))
  ...
  fields_info <- stringdist_left_join_with_checks(
    ...
    dplyr::rename_with( ~ stringr::str_replace(.x, "child_", "field_")) |> dplyr::rename(field_exact_match =
                                                                                           exact_match)
  pot_valueset_info <- purrr::map(
    fields_info$field_location,
    ~ get_target_type_info_below(.url = .x, .target_types = "VALUESET") |> dplyr::mutate(field_location =
                                                                                           .x)
  ) |> dplyr::bind_rows() |>
    dplyr::select(-type) |> dplyr::rename_with( ~ paste0("valueset_", .x), -field_location) |> dplyr::left_join(fields_info, by =
                                                                                                                  c("field_location")) |> dplyr::select(-dplyr::ends_with(".input"))
  pot_values_info <- purrr::map(pot_valueset_info$valueset_location,
                                ~ get_schema_info(url = .x)) |> dplyr::bind_rows() |>
    dplyr::select(-type) |> dplyr::rename_with( ~ stringr::str_replace(.x, "child_", "value_")) |> dplyr::select(dplyr::starts_with("value_"), valueset_location =
                                                                                                                   location) |>
  ...
  value_label_rows <- value_labels_tbl |> dplyr::filter(!is.na(value_label) &
                                                          is.na(value_code)) |>
    stringdist_left_join_with_checks(
      pot_values_info,
  ...
  value_tbl <- dplyr::bind_rows(value_label_rows, value_code_rows) |> dplyr::select(-dplyr::starts_with("field_")) |> dplyr::rename(value_exact_match =
                                                                                                                                      exact_match)
