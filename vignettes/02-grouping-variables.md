...
# Each row now has human-readable labels alongside the IDs
spec_tbl_labelled |>
...
spec_tbl_grouped <- spec_tbl_labelled |>
  mutate(
    value_group = case_when(
      # Age bands – use value_code for numeric comparison
      ...
      # Leave all other fields ungrouped
      TRUE ~ NA_character_
    )
  )
...
spec_tbl_grouped <- spec_tbl_labelled |>
  mutate(
    value_group = case_when(
      ...
      field_label == "Age of Claimant (Single Years)" &
        as.integer(value_code) >= 55      ~ "55+",
      # Date periods – group by month label
      field_label == "Month" &
        str_detect(value_label, "2013|2019") ~ "Pre-2022",
      field_label == "Month" &
        str_detect(value_label, "2022")      ~ "2022",
      TRUE ~ NA_character_
    )
  )
```
