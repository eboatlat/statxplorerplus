...
3.  Remove the most recent date to simulate a spec that is not fully up
    to date.
4.  Update the spec to include all dates newer than those already in the
    spec.
...
To demonstrate what happens when new data becomes available, remove the
most recent date from the spec. This simulates a spec that was saved
...
The data now covers all August 2013 and August 2019, which were in the original query, and then each of the newer months of data after August 2019 giving 38 months, each with 306 rows (51 age values ×
